...
import grts.core.architecture.Battery;
...
/**
 * Created by Bastien on 05/05/2016.
 */
public class EnergyMonoProcessor implements IProcessorPolicy {
    ...
    /**
     * Creates a new mono processor policy with energy consumption using PFPasap scheduling algorithm.
     *
     * @param policy The priority policy associated with the mono processor policy.
     */
    public EnergyMonoProcessor(Architecture architecture, IPriorityPolicy policy) {
...
//        this.processor = new Processor(0);
    ...
    public List<AbstractMap.SimpleEntry<Job, Integer>> chooseNextJobs(long time) {
        Battery battery = architecture.getBattery().get();
        battery.reload();
        ...
        PeriodicTaskEnergyAware periodicTaskEnergyAware = (PeriodicTaskEnergyAware) jobToExecute.getTask();
        long consumption = periodicTaskEnergyAware.getWcec() / periodicTaskEnergyAware.getWcet();
        // if there not enough energy
        if (battery.getStatus() < consumption) {
            return Collections.emptyList();
        }
        // otherwise energy is consumed
